...
public class bankSystem {
	...
	public static void managerPrompt(Statement statement) throws Exception{ //opens up the manager interface
		int customerID, balance, amount, depoAmount, accountNum, accountNumz, overdraft, interestFee, accountFee;
		String type;
		int min = 100000;  
		int max = 1000000;
		...
		switch(userResponse) { //another switch case over user input
			...
			case 5:{ //Lets manager transfer money after info collection
				System.out.println("What is the account number?");
				accountNum = scan.nextInt();
				System.out.println("What is the account number of the account that the money is being sent to?");
				accountNumz = scan.nextInt();
			...
			case 6:{
				System.out.println("What is the account number?");
				accountNum = scan.nextInt();
				System.out.println("What do you want the overDraft fee to be?");
				overdraft = scan.nextInt();
				alterOverdraft(accountNum, overdraft, statement);
				break;
			}
			case 7: {
				System.out.println("What is the account number?");
				accountNum = scan.nextInt();
				System.out.println("What do you want the interest fee to be?");
				interestFee = scan.nextInt();
				alterInterest(accountNum, interestFee, statement);
				break;
			}
			case 8: {
				System.out.println("What is the account number?");
				accountNum = scan.nextInt();
				System.out.println("What do you want the account fee to be?");
				accountFee = scan.nextInt();
				alterAccount(accountNum, accountFee, statement);
				break;
			}
			case 9: {
				System.out.println("What is the account number?");
	...
	public static void transferMoney(int customerID, int accountNum, int accountNumz, int amount, Statement statement) throws Exception {
        withdraw(customerID, accountNum, amount, statement); //calls upon withdraw method
        deposit(accountNumz, amount, statement); //calls upon deposit method
    }
	
	public static void alterOverdraft(int accountNum, int overdraft, Statement stmt) throws Exception { //allows the manager to update overDraftfee on the account through update statement
		String overdraftz = "UPDATE accounts SET overdraftFee=" + overdraft;
		statement.executeUpdate(overdraftz);
	}
	
	public static void alterInterest(int accountNum, int interest, Statement stmt) throws Exception { //allows the manager to update interestFee on account through Update statement
		String interestFee = "UPDATE accounts SET overdraftFee=" + interest;
		statement.executeUpdate(interestFee);
	}
	
	public static void alterAccount(int accountNum, int accountFee, Statement stmt) throws Exception { //allows the manager to update accountFee on account through Update statement
		String accountFeez = "UPDATE accounts SET overdraftFee=" + accountFee;
		statement.executeUpdate(accountFeez);
	}
